...
0000000000400ee0 <phase_1>:
  ...
  400ee9:	e8 4a 04 00 00       	callq  401338 <strings_not_equal>   ;调用函数strigns_not_equal,其中第一个参数为phase_1的参数input,第二个参数为0x402400(%)
...
0000000000400fce <func4>:                                           ; func4(a=[rsp+8],0,14) 设输入的第一个数据为14，即a=14
  400fce:	48 83 ec 08          	sub    $0x8,%rsp                    ; rsp-=8                         
  400fd2:	89 d0                	mov    %edx,%eax                    ; eax = edx(14)         eax=14
  400fd4:	29 f0                	sub    %esi,%eax                    ; eax -= esi(0)         eax=14  6
  400fd6:	89 c1                	mov    %eax,%ecx                    ; ecx = eax             ecx=14  6
  400fd8:	c1 e9 1f             	shr    $0x1f,%ecx                   ; ecx >>= 31 （逻辑右移） ecx=0  0
  400fdb:	01 c8                	add    %ecx,%eax                    ; eax += ecx            eax=14  13
  400fdd:	d1 f8                	sar    %eax                         ; 只有一个操作数是省略了1, 即eax>>=1  eax=7 
  400fdf:	8d 0c 30             	lea    (%rax,%rsi,1),%ecx           ; ecx = [rax]+[rsi]                 ecx=7 7
...
0000000000401062 <phase_5>:
  401062:	53                   	push   %rbx           ; ebx bx bl rdi edi di
  ...
  40108f:	88 0c 24             	mov    %cl,(%rsp)                     ; [rsp]=cl    rcx ecx esi rsi
...
0000000000401204 <fun7>:                                    ; 假设fun7(0x6030f0,num)  我们的secret_phase要求返回2
...
0000000000401242 <secret_phase>:
  401242:	53                   	push   %rbx
  401243:	e8 56 02 00 00       	callq  40149e <read_line>         ; 读一行，应该是要我们输入字符串
...
000000000040145c <read_six_numbers>:                          ; 调用函数read_six_numbers(char* input,rsi)
  40145c:	48 83 ec 18          	sub    $0x18,%rsp              ; rep-=24     开24字节空间   int b[6];  
  ...
  40146b:	48 89 44 24 08       	mov    %rax,0x8(%rsp)          ; rax -> rsp+8   b[2]        b[2]=rax;  
